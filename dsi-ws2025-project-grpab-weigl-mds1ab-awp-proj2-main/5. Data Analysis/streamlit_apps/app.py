# app.py — Unified Streamlit application (Home + Geo + Funding + Survivorship + Industry/NLP + Status)
# Location: 5. Data Analysis/streamlit_apps/app.py
...
# ----------------------------
def render_home():
    # Custom CSS (kept from your landing.py)
    st.markdown("""
    <style>
    .block-container {
        padding-top: 1rem;
    ...
    with col_image:
        # Keep your relative path convention
...
def load_investment_data():
    # NOTE: expects table country_investments already created/loaded
    # and columns: (id, business_stage, country_code, year, amount)
    query = """
    SELECT
        ci.id,
        ci.business_stage,
...
# Survivorship page (split from same script)
...
def render_industry():
    ...
    with tab1:
        ...
        with col2:
            ...
            fig, ax = plt.subplots(figsize=(8, 8))
            sns.barplot(data=top_median, y="industries", x="median_funding", ax=ax)
    ...
    with tab2:
        ...
        selected_cluster = st.selectbox("Select a cluster to inspect:", options=cluster_ids, index=0)
        ...
        fig, ax = plt.subplots(figsize=(10, 5))
        sns.barplot(data=top_funding, x="cluster", y="median_funding", ax=ax)
        ...
        popular_inds = (
            cluster_industry_counts.groupby("industries_list")["count"]
            .sum()
            .sort_values(ascending=False)
            .head(25).index
        )
        pivot_ci = (
            cluster_industry_counts[cluster_industry_counts["industries_list"].isin(popular_inds)]
            .pivot(index="cluster", columns="industries_list", values="count")
            .fillna(0)
        )
        fig, ax = plt.subplots(figsize=(14, 6))
        sns.heatmap(pivot_ci, linewidths=0.2, ax=ax)
...
def render_status():
    ...
    if not df_funded.empty:
        fig, ax = plt.subplots(figsize=(10, 5))
        sns.scatterplot(data=df_funded, x="age", y="funding_amount", hue="lifecycle_status", hue_order=status_options, alpha=0.6, ax=ax)
        ax.set_yscale("log")
        ax.set_xlabel("Age (years)")
    ...
    min_companies_country = st.slider(
        "Minimum number of companies per country",
        min_value=50,
        max_value=int(country_pivot["n_companies"].max()) if len(country_pivot) else 50,
        value=200,
        step=50
    )

    country_plot_df = country_pivot[country_pivot["n_companies"] >= min_companies_country].copy()
    country_plot_df = country_plot_df.sort_values("active", ascending=False)

    fig, ax = plt.subplots(figsize=(12, 6))
    ax.bar(country_plot_df["country"], country_plot_df["active"], label="Active ratio")
    ax.bar(country_plot_df["country"], country_plot_df["inactive"], bottom=country_plot_df["active"], label="Inactive ratio")
    ax.set_ylim(0, 1)
    ax.set_ylabel("Ratio")
